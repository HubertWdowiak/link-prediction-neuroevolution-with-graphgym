    ...
    def __init__(
        ...
        **kwargs
        ...
        if cfg.gnn.dropout > 0:
            layer_wrapper.append(nn.Dropout(p=cfg.gnn.dropout, inplace=cfg.mem.inplace))
        if has_act:
...
class Linear(nn.Module):
    ...
    def forward(self, batch):
        if isinstance(batch, torch.Tensor):
            ...
            # convert batch.x to the same dtype as self.model.weight
            batch.x = batch.x.to(self.model.weight.dtype)
...
class SplineConv(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        ...
        self.model = pyg.nn.SplineConv(dim_in, dim_out, dim=1, kernel_size=2, bias=bias)
