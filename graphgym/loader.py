...
from graphgym.models.transform import edge_nets, ego_nets, path_len, remove_node_feature
...
def load_dataset():
    ...
    # Load from OGB formatted data
        ...
        elif cfg.dataset.name == "ogbl-ddi":
            dataset = PygLinkPropPredDataset(name=name, root=dataset_dir)
            graphs = GraphDataset.pyg_to_graphs(dataset, fixed_split=True, verbose=True)
...
def transform_before_split(dataset):
    ...
    if cfg.dataset.remove_feature:
        dataset.apply_transform(
            remove_node_feature, update_graph=True, update_tensor=False
        )
    ...
    if cfg.dataset.augment_label:
        dataset.apply_transform(
            preprocess._replace_label, update_graph=True, update_tensor=False
        )
    ...
    if cfg.dataset.task == "edge" and "id" in cfg.gnn.layer_type:
        dataset.apply_transform(path_len, update_graph=False, update_tensor=False)
...
def transform_after_split(datasets):
    ...
    if cfg.dataset.transform == "ego":
        ...
        for split_dataset in datasets:
            split_dataset.apply_transform(
                edge_nets, update_tensor=True, update_graph=False
            )
...
def set_dataset_info(datasets):
    ...
    try:
        ...
        if "classification" in cfg.dataset.task_type and cfg.share.dim_out == 2:
            cfg.share.dim_out = 1
    except Exception:
        cfg.share.dim_out = 1
